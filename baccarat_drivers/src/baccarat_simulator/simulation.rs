...
impl<'a> SimulatorGambler<'a> {
    fn new(
        ...
        Self {
            ...
            rounds: 0,
            main_bet_money: config.main_bet_base,
            side_bet_money: config.side_bet_base,
...
impl<'a> GamblerProvider for SimulatorGambler<'a> {
    ...
    fn on_round_end(&mut self, player: &Hand, banker: &Hand, round_result: &RoundResult) {
        ...
        // Change main bet base
        if self.config.main_bet_double_when_lose {
            for (bet, bet_result) in &round_result.details {
                match *bet {
                    HandsBet::PlayerWin | HandsBet::BankerWin | HandsBet::Tie => {
                        if bet_result.1 > 0 {
                            self.main_bet_money = self.config.main_bet_base;
                        } else if bet_result.1 < 0 {
                            self.main_bet_money *= 2;
                        }
                        break;
                    }
                    _ => {}
